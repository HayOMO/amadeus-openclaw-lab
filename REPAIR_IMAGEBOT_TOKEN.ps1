...
Write-Host "Repair Telegram imagebot token storage"
Write-Host "This will store the token in ~\.openclaw\secrets\telegram-imagebot.token and point OpenClaw at that file."
Write-Host ""

$secure = Read-Host "Paste NEW Telegram BotFather token (input hidden)" -AsSecureString
...
try {
  ...
  & openclaw config unset channels.telegram.accounts.default *> $null
  if ($LASTEXITCODE -ne 0) {
    Write-Host "No stale default Telegram account to remove."
  }

  & openclaw config unset channels.telegram.botToken *> $null
  if ($LASTEXITCODE -ne 0) {
    Write-Host "No stale root Telegram botToken to remove."
  }
...
'@
  ...
  Write-Host ""
  Write-Host "Token storage repaired. Start gateway with:"
