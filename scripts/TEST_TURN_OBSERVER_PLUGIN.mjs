...
assert.match(__testing.summarizeParams({ query: "search C:\\Users\\Bot\\secret.txt", count: 3 }), /\[local-path-redacted\]/);
assert.match(__testing.summarizeParams({ query: "search C:\\Users\\Bot\\secret.txt", count: 3 }), /"count":3/);
...
const runtimeCtx = { agentId: "imagebot", sessionKey: "s1", runId: "r1", modelId: "gpt-test" };

await beforePrompt({ text: "助手 查一下 C:\\Users\\Bot\\secret.txt", chatId: "-100", userId: "42" }, runtimeCtx);
await beforeTool({ toolName: "web_search", toolCallId: "t1", params: { query: "test C:\\Users\\Bot\\secret.txt" } }, runtimeCtx);
await afterTool({ toolName: "web_search", toolCallId: "t1", result: { details: { status: "ok" }, content: [{ type: "text", text: "done" }] } }, runtimeCtx);
...
assert.equal(toolsOnly.details.results[0].modelId, "gpt-test");
assert.match(toolsOnly.details.results[0].paramsPreview, /"query":"test \[local-path-redacted\]"/);
assert.match(toolsOnly.content[0].text, /model=gpt-test/);
assert.ok(!toolsOnly.content[0].text.includes("C:\\Users\\Bot"));
