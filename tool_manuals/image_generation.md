...
keywords: draw, generate, image2, gpt-image-2, image model, edit image, reference image, redraw, prompt, character reference, artist style, 生图, 画图, 生成图片, 改图, 重画, 重新画, 参考图, 人设图, 官方人设, 原设画风, 画师, 上张图, 重新发, 串味
...
- `image`: load extra image paths/URLs that are not already visible; native
  prompt images should be inspected directly. It does not draw.
...
- If image generation fails/times out/aborts: report the concrete failure reason returned by the tool/completion event. Retry once only when the same local reference images are still required and the failure looks like transport/input delivery. Otherwise stop and reply briefly. No downgrade loop.
...
- A generated image from the current turn is not a default reference for the next `image_generate` call. Use it only when the user clearly wants to edit/continue that exact generated image; if the result was bad, go back to the original request, source reference, or prompt rather than using the bad output as the new base.
...
- For named characters, public people, VTubers, mascots, OCs, products, places, logos, or unfamiliar visual subjects:
  ...
  2. public/current search if needed, prioritizing official/canonical design sources
  3. `web_image_search`
...
- Before generation, verify the character's source title and visible design cues: hair, eyes, outfit, silhouette, signature accessories, and common wrong variants. If no official image is available, use a trusted wiki/gallery/reference and keep the prompt honest about uncertainty.
- Text-only generation is acceptable only when no reliable reference is available or user asks for prompt-only.

## Named Artist / Style

- If the user specifies an artist, illustrator, studio, or very specific style, search representative public works before generation unless the style is already saved in `image_skill`.
- Prefer the artist's official pages, Pixiv/profile pages, art-book/portfolio pages, Danbooru artist-tag pages, or other source pages with visible works. Verify at least the recurring linework, color, face/eye shape, lighting, composition, texture, and subject tendencies.
- Convert the verified style signals into prompt language. Do not rely on the artist name alone, and do not pass unrelated generated images as a style reference.
...
- `recipe.artist_style_reference`: named artist/style requests that need
  representative work lookup and verified style cues.
