...
tools: knowledge_sources, knowledge_search, knowledge_recent, knowledge_ingest, memory_search, persona_search
...
`knowledge_search` is a lightweight RAG-style registry over:
...
Use `knowledge_search` when the source is broad or unclear.
...
- `action:delete`: dry-run by default; `dryRun:false` deletes a visible
  ingested record and records a delete event.

Persisted notes are stored under the current scope. `knowledge_search` and
`knowledge_recent` only return same-scope `user_docs` during normal runtime
calls. No-context local maintenance can still inspect legacy/admin docs.

## Recent And Sources

Use `knowledge_sources` to inspect registered sources. Use `knowledge_recent` to find recently saved notes/docs.
